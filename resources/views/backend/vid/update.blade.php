...
<div class="container-fluid px-4">
    <ol class="breadcrumb mb-4">
        <h1 class="breadcrumb-item active">Edit Video</h1>
        <hr>
    </ol>

    <form action="{{route('be.videoupdate', $load->id)}}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="mb-3 row">
            <label for="inputPassword" class="col-sm-2 col-form-label">Link Video</label>
            <div class="col-sm-5">
                <input type="text" class="form-control" name="link" value="{{old('link',isset($load->link)?$load->link:null)}}">
                {!!$errors->first("link",' <div class="text-danger">:message
